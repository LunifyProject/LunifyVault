...
public class BarcodeData {
    ...
    public static final String XLA_SCHEME = "lunify:";
    ...
    public static final String OA_LFI_ASSET = "lfi";
    ...
    static public BarcodeData fromQrCode(String qrCode) {
        // check for lunify uri
        BarcodeData bcData = parseLunifyUri(qrCode);
        // check for naked lunify address / integrated address
        if (bcData == null) {
            bcData = parseLunifyNaked(qrCode);
        }
        // check for btc uri
    ...
    static public BarcodeData parseOpenAlias(String oaString, boolean dnssec) {
        ...
        Asset asset;
        if (OA_LFI_ASSET.equals(oaAsset)) {
            if (!Wallet.isAddressValid(address)) {
                Timber.d("XLA address invalid");
                return null;
            }
