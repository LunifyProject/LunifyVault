...
import xyz.lunify.levin.scanner.Dispatcher;
import xyz.lunify.levin.scanner.LevinPeer;
    ...
    private boolean testRpcService(int port) {
        ...
        try {
            ...
            try (Response response = client.newCall(request).execute()) {
                if(getHost().contains("remote.") && getHost().contains("lunify")) // always prioritize lunify nodes
                    responseTime = 0;
                else
